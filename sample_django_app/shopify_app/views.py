...
class LoginView(View):
    ...
    def get(self, request, *args, **kwargs):
        # print("arg:   ", request.GET)
        if request.GET.get("shop"):
            # print("AUTHENTICATING.......")
            return authenticate(request)
        return render(
            request, "shopify_app/login.html", {"app_name": "Custom Django"}
        )
...
def callback(request):
    ...
    try:
        ...
        access_token, access_scopes = exchange_code_for_access_token(
            request, shop)
...
def authenticate(request):
    try:
        ...
        # print("OOPS EXCEPTION!! : ", str(exception))
        messages.error(request, str(exception))
        return redirect(reverse("login"))
...
def after_authenticate_jobs(shop, access_token):
    ...
    # ++++=====================================================++++ >> KJ
    remove_all_webhooks(shop, access_token)
    create_product_delete_webhook(shop, access_token)
    create_order_fulfilled_webhook(shop, access_token)
    # ++++=====================================================++++ >> KJ
...
# ++++=====================================================++++ >> KJ
# Manually Created
# Function to get all registered webhooks - KJ


def get_all_webhooks(shop, access_token):
    ...
    response = requests.get(
        f'https://{shop}/admin/api/2023-07/webhooks.json', headers=headers)

    return json.loads(response.text)

# function to remove all registred webhooks - KJ


def remove_all_webhooks(shop, access_token):
    with shopify_session(shop, access_token):
        all_webhooks = get_all_webhooks(shop, access_token)
        for webhook in all_webhooks['webhooks']:
            shopify.Webhook.delete(webhook['id'],)


def create_product_delete_webhook(shop, access_token):
    with shopify_session(shop, access_token):
        app_url = apps.get_app_config("shopify_app").APP_URL
        webhook = shopify.Webhook()
        print("========PRODUCTS/DELETE WEBHOOK CREATED===========")
        webhook.topic = "products/delete"
        webhook.address = "https://{host}/webhook/product_delete".format(
            host=app_url)
        webhook.format = "json"
        webhook.save()


def create_order_fulfilled_webhook(shop, access_token):
    with shopify_session(shop, access_token):
        app_url = apps.get_app_config("shopify_app").APP_URL
        webhook = shopify.Webhook()
        print("========Order/Fulfilled WEBHOOK CREATED===========")
        webhook.topic = "orders/fulfilled"
        webhook.address = "https://{host}/webhook/order_fulfilled".format(
            host=app_url)
        webhook.format = "json"
        webhook.save()
